...
const removeContact = (io)=>{//Trước hết giả sử người A (userId) gửi lời mời cho người B (contactId) 
  let clients = {};
  io.on('connection',(socket)=>{
     const currentUserId = socket.request.user._id;     
     clients = pushSocketIdToArray(clients,currentUserId,socket.id);
     socket.on("user-remove-contact",(data)=>{//Server nhận được sự kiện từ client gửi lên
